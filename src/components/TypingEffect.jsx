...
const TypingEffect = () => {
  ...
  useEffect(() => {
    ...
    const WORD_MISTAKE = 'const job = "Prograa";';
    const WORD_STEM = 'const job = "Progra';
    const WORD_FINAL = 'const job = "Programador";';
    ...
    const loop = () => {
      ...
      if (state.phase === 0) { // Typing "...Prograa..."
      ...
      } else if (state.phase === 1) { // Deleting
      ...
      } else if (state.phase === 2) { // Typing Correct
  ...
  const renderHighlightedText = (currentText) => {
    ...
    if (remaining.startsWith("const")) {
      ...
      parts.push(<span key="const" style={{ color: '#c678dd' }}>{remaining.substring(0, len)}</span>);
    ...
    if (remaining.length > 0 && !remaining.startsWith("=") && !remaining.startsWith("\"")) {
       ...
       if (match) {
         parts.push(<span key="var" style={{ color: '#61afef' }}>{match[1]}</span>);
    ...
    // Space and equals
    if (remaining.startsWith(" = ")) {
       parts.push(<span key="eq" style={{ color: '#56b6c2' }}> = </span>);
       remaining = remaining.substring(3);
    } else if (remaining.startsWith(" =")) {
       parts.push(<span key="eq-partial" style={{ color: '#56b6c2' }}> =</span>);
       remaining = remaining.substring(2);
    } else if (remaining.startsWith(" ")) {
       parts.push(<span key="sp2"> </span>);
       remaining = remaining.substring(1);
       if (remaining.startsWith("=")) {
          parts.push(<span key="eq-only" style={{ color: '#56b6c2' }}>=</span>);
          remaining = remaining.substring(1);
       }
    }
    ...
    if (remaining.length > 0) {
       parts.push(<span key="string" style={{ color: '#98c379' }}>{remaining}</span>);
    }
  ...
  return (
    ...
    >
      ...
      <span className="cursor" style={{ opacity: 1, animation: 'blink 1s step-end infinite', color: 'white' }}>|</span>
      <style>{`
